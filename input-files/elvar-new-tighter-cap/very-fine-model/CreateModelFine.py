...
cols = [col for col in geo.columnlist if 0. <= col.centre[0] <= 1.e2]
...
layer = geo.layerlist[-1] # bottom layer
cols = [col for col in geo.columnlist if 1.e2 <= col.centre[0] <= 2.e3]
totalarea = sum([col.area for col in cols])
for col in cols:
    blockname = geo.block_name(layer.name, col.name)
