...
class ModelDiscrep:
    ...
    def compute_raw_model_discrepancy(self, num_runs=1, n_ignore=1, save_data=True):
        ...
        while i < num_runs:
            ...
            self.discrepancy_dist[i, :] = self.process_space.resid
...
class BayesModel:
    ...
    def predictive_checks(self, parameter_sets, subtract_bias=True, n_ignore=1, project_data_to_process=True,
        ...
        if plot_wells_list is None:
            plot_wells_list = list(obs_wells_list)
            #obs_wells_list = filter(r.match, self.process_model.geom.well.keys())

        #first plot true 'data': either actual or process. 
        ...
        for i,welli in enumerate(obs_wells_list):
            if project_data_to_process:
                ...
                #(d_measured, temps_measured) = geom_measured.well_values(welli, temps_measured, elevation=True)
                if welli in plot_wells_list:
        ...
        for param_set in parameter_sets:
            ...
            #bias_function = self.measurement_space.bias_function
            
            #--easier for predictive checks to use original discrepancy model? Always in model space?--
            #if map_discrep_to_data:
            #    discrep = self.discrepancy_model.discrepancy_dist_data
            #    discrep_starting_index = [len(self.data_model.ss_temps_obs_well[key])-n_ignore for key in np.arange(len(obs_wells_list))]
            #    discrep_starting_index = [0] + list(np.cumsum(discrep_starting_index))
            #else:
            ...
            #lift to data space if necessary?
            for i,welli in enumerate(obs_wells_list):
                ...
                if subtract_bias:
                    ...
                    if np.asarray(self.process_space.discrepancies).shape==():
                        plt.plot(temp_model[n_ignore:]-self.process_space.discrepancies,d_model[n_ignore:],'-',color='b',alpha=alpha) 
                    else:
