...
class GeoModel:
    ...
    def set_rock_permeabilities(self, perm_powers):
        ...
        if self.islayered:
            ...
            for i, rt in enumerate(self.datfile.grid.rocktypelist):
                if i > 0: #avoid setting atmosphere permeability
                    #print(i, rt)
    ...
    def set_generator_params(self, gen_params): 
        '''
        added by SWS self updates:
            datfile -> datfile (permeabilities)
        '''
        #datfile = t2data(self.datfile_name + '.dat')
    ...
    def simulate(self, is_silent=False, reshape=False, do_update_obs_wells=True):
        '''
        self updates:
            results -> results
            ss_temps -> ss_temps
        ...
        if platform.system() =='Linux':
            ...
            bad_model = self.datfile.run(save_filename='output-files/saved_run/save.save', incon_filename=self.incon_name, simulator='AUTOUGH2_SC_D64', silent=False)
            stop = timeit.default_timer()
            print('time to run model (s): ', stop-start)
            if bad_model:
                 self.is_good_model = False
                 self.ss_temps = np.zeros(int(self.geom.num_layers * self.geom.num_columns))
            else:
                newfn = 'krafla_vcoarselast.listing'
