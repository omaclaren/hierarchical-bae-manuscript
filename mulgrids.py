...
class mulgrid(object):
        ...
        if line_length > 0.0:
            for i in range(divisions + 1):
                ...
                if blkname:
                    ...
                    bn.append(blkname)
                    #print(blkname)
        return np.array(x), np.array(y), np.array(bn)
    ...
    def polyline_values(self, polyline, variable, divisions = 100,
        ...
        for i in range(len(polyline) - 1):
            ...
            xi, yi, bni = self.line_values(start, end, variable, divisions, coordinate, qtree = qtree)
            if i > 0:
                xi = xi[1:]; yi = yi[1:]; bni = bni[1:]; 
            if not coordinate:
                if len(x) > 0: xi += x[-1]  # add end distance from last segment
            x += list(xi)
            y += list(yi)
            bn = np.append(bn, bni)
            #print('i = ', i, 'y = ', y, 'blockname = ', bn)
        return np.array(x), np.array(y)#, np.array(bn)
